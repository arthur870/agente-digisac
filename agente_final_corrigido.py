...
HORARIO_FIM = 22    # 22h (10pm)
FUSO_HORARIO = "America/Sao_Paulo"  # GMT-3 (Brasília)
...
def verificar_horario_funcionamento():
    ...
    if HORARIO_INICIO <= hora_atual < HORARIO_FIM:
        ...
        return False, f"Horário de funcionamento: {HORARIO_INICIO}h às {HORARIO_FIM}h"

def mensagem_fora_horario():
    """Retorna mensagem para enviar fora do horário"""
    return f"🕒 Olá! Nosso atendimento funciona das {HORARIO_INICIO}h às {HORARIO_FIM}h. Deixe sua mensagem que responderemos assim que possível! 😊"
...
def webhook():
    ...
    try:
        ...
        # ✅ GARANTIR MODO REATIVO: SÓ responde a mensagens DO CLIENTE
        if not mensagem_texto or mensagem_texto.strip() == "":
            log("⏭️ Mensagem vazia ou sem texto, ignorando")
            return jsonify({"status": "empty_message"}), 200
        
        log(f"💬 Mensagem do cliente detectada: '{mensagem_texto[:50]}...'")
        
        # 🕒 VERIFICAR HORÁRIO DE FUNCIONAMENTO
        dentro_horario, msg_horario = verificar_horario_funcionamento()
        
        if not dentro_horario:
            log(f"⏰ Fora do horário de funcionamento - {msg_horario}")
            ...
            log("✅ Mensagem de horário enviada")
            return jsonify({"status": "fora_horario"}), 200
        
        log(f"✅ Dentro do horário de funcionamento")
