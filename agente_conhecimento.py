...
def buscar_conhecimento(pergunta, max_resultados=5):
    """Busca conhecimentos relevantes na base com priorização de regras comerciais"""
    ...
    for conhecimento in ativos:
        ...
        if categoria == 'comercial':
            score += 15  # Priorizar regras comerciais
    ...
    log(f"🔍 Busca: '{pergunta[:50]}...' → {len(top_resultados)} resultados (comercial: {eh_comercial})")
...
- Priorize respostas curtas e objetivas (2-4 linhas)
...
Você: "O prazo médio de entrega é de 30 dias úteis após aprovação do pedido e confirmação de pagamento. Esse prazo pode variar conforme complexidade do projeto."
