...
def verificar_atendente_humano_no_webhook(dados):
    ...
    # Verificar ticketUserId (atendente atribuído ao ticket)
    if evento == 'message.created':
        ticket_user_id = data.get('ticketUserId')
        
        # Se ticket tem usuário atribuído = atendente ativo
        if ticket_user_id and ticket_user_id != DIGISAC_USER_ID:
            log(f"👤 Ticket com atendente atribuído: ticketUserId={ticket_user_id}")
            return True, ticket_user_id
    ...
    if evento == 'message.created':
        ...
        # ✅ CRITÉRIO PRINCIPAL: isFromMe=true + isFromBot=false = Atendente humano
        if is_from_me and not is_from_bot:
            log(f"👤 Mensagem de atendente humano detectada (isFromMe=true, isFromBot=false)")
            if user_id:
...
def webhook():
    ...
    try:
        ...
        # ✅ VERIFICAR ÁREA: Bot atua apenas em Fila/Contatos (sem ticketUserId)
        ticket_user_id = data.get('ticketUserId')
